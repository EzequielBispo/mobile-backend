...
db.serialize(() => {
    db.run("CREATE TABLE IF NOT EXISTS tarefas (id INTEGER PRIMARY KEY, 
tarefa TEXT)");
});

// Criar a tabela 'usuarios' no banco de dados
db.serialize(() => {
    db.run("CREATE TABLE IF NOT EXISTS usuarios (id INTEGER PRIMARY KEY, 
...
const verificarToken = (req, res, next) => {
    ...
    jwt.verify(token.split(' ')[1], 'secreto', (err, decoded) => {
        if (err) {
            return res.status(500).json({ error: 'Falha ao autenticar o 
token.' });
        }
...
app.post('/tarefas', verificarToken, (req, res) => {
    ...
    db.run("INSERT INTO tarefas (tarefa) VALUES (?)", [tarefa], 
function(err) {
        if (err) {
...
app.put('/tarefas/:id', verificarToken, (req, res) => {
    ...
    db.run("UPDATE tarefas SET tarefa = ? WHERE id = ?", [tarefa, id], 
function(err) {
        if (err) {
            return res.status(500).json({ error: err.message });
        }
        if (this.changes) {
            res.status(200).json({ message: 'Tarefa atualizada com 
sucesso!' });
        } else {
            res.status(404).json({ error: 'Tarefa não encontrada!' });
        }
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const senhaValida = await bcrypt.compare(password, 
usuario.password);
        if (!senhaValida) {
            return res.status(401).json({ error: 'Senha incorreta' });
        }
        const token = jwt.sign({ id: usuario.id, username: 
usuario.username, role: usuario.role }, 'secreto', { expiresIn: '1h' });
...
const buscarUsuario = (username) => {
    return new Promise((resolve, reject) => {
        db.get('SELECT * FROM usuarios WHERE username = ?', [username], 
(err, row) => {
            if (err) {
                reject(err);
            }
